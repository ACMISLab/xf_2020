...
from ..layers.attentional_feature_fusion import AFF
from ..layers.axial_attention import AxialAttention
from ..layers.afam import AFAM
from ..layers.cbam import CBAM
from ..layers.chanel_spatial_joint_attention import CSAM
from ..layers.feature_fusion import SelectiveFeatureFusionModule
...
@MODELS.register_module()
class PVTv2AxAN(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2AxAN, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            use_conv_ffn=True,
            **kwargs)
        self.axan0 = AxialAttention(in_dim=64)
        self.axan1 = AxialAttention(in_dim=128)
        self.axan2 = AxialAttention(in_dim=320)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < 3:
                am_layer = getattr(self, f'axan{i}')
                x = am_layer(x)
...
@MODELS.register_module()
class PVTv2AFAM(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2AFAM, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            use_conv_ffn=True,
            **kwargs)
        self.afam0 = AFAM(channels=64)
        self.afam1 = AFAM(channels=128)
        self.afam2 = AFAM(channels=320)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < 3:
                am_layer = getattr(self, f'afam{i}')
                x = am_layer(x)
...
@MODELS.register_module()
class PVTv2CBAM(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2CBAM, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            use_conv_ffn=True,
            **kwargs)
        # self.am0 = CSAM(in_channels=64)
        self.am1 = CBAM(in_channels=128)
        self.am2 = CBAM(in_channels=320)
        self.am3 = CBAM(in_channels=512)
...
@MODELS.register_module()
class PVTv2CSAM(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2CSAM, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            ...
            **kwargs)
        # self.am0 = CSAM(in_channels=64)
        self.am1 = CSAM(in_channels=128)
        self.am2 = CSAM(in_channels=320)
        self.am3 = CSAM(in_channels=512)
...
@MODELS.register_module()
class PVTv2SE(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2SE, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            use_conv_ffn=True,
            **kwargs)
        self.am0 = SELayer(channels=64)
        self.am1 = SELayer(channels=128)
        self.am2 = SELayer(channels=320)
...
@MODELS.register_module()
class PVTv2SFM(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2SFM, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            use_conv_ffn=True,
            **kwargs)
        self.sfm = SelectiveFeatureFusionModule(spa_channels=128, sem_channels=512)
    ...
    def forward(self, x):
        ...
        outs[1] = self.sfm(outs[1], outs[3])

        return outs


@MODELS.register_module()
class PVTv2AFF(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2AFF, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
            use_conv_ffn=True,
            **kwargs)
        self.aff = AFF(spa_channels=128, sem_channels=512)
    ...
    def forward(self, x):
        ...
        outs[1] = self.aff(outs[1], outs[3])

        return outs


@MODELS.register_module()
class PVTv2AFM(PyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(PVTv2AFM, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
            use_abs_pos_embed=False,
            norm_after_stage=True,
